...
template <int Dim>
void MPMapUtil<Dim>::setPotentialRadius(const Vecf<Dim>& radius) {
  potential_radius_ = radius;
}

template <int Dim>
void MPMapUtil<Dim>::setPotentialMapRange(const Vecf<Dim>& range) {
  potential_map_range_ = range;
}

template <int Dim>
void MPMapUtil<Dim>::setPotentialWeight(decimal_t w) {
  this->ENV_->set_potential_weight(w);
}

template <int Dim>
void MPMapUtil<Dim>::setGradientWeight(decimal_t w) {
...
template <int Dim>
vec_Vec3f MPMapUtil<Dim>::getPotentialCloud(decimal_t h_max) {
...
template <int Dim>
vec_Vec3f MPMapUtil<Dim>::getGradientCloud(decimal_t h_max, int i) {
  const auto data = this->map_util_->getMap();
  const auto dim = this->map_util_->getDim();
  const decimal_t ratio = h_max / H_MAX;
  vec_Vec3f ps;
...
template <int Dim>
vec_E<Vecf<Dim>> MPMapUtil<Dim>::calculateGradient(const Veci<Dim>& coord1, const Veci<Dim>& coord2) {
  const auto dim = this->map_util_->getDim();
...
template <int Dim>
void MPMapUtil<Dim>::createMask(int pow) {
...
template <int Dim>
void MPMapUtil<Dim>::updatePotentialMap(const Vecf<Dim>& pos) {
  createMask(1);
