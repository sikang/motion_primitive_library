...
template <int Dim> MPBaseUtil<Dim>::MPBaseUtil() { planner_verbose_ = false; }

template <int Dim> bool MPBaseUtil<Dim>::initialized() {
  return !(ss_ptr_ == nullptr);
}

template <int Dim> void MPBaseUtil<Dim>::reset() {
  ss_ptr_ = nullptr;
  traj_ = Trajectory<Dim>();
  ws_.clear();
}

template <int Dim> void MPBaseUtil<Dim>::setLPAstar(bool use_lpastar) {
...
template <int Dim> void MPBaseUtil<Dim>::setEpsilon(decimal_t eps) {
  epsilon_ = eps;
  if (planner_verbose_)
    printf("[MPBaseUtil] set epsilon: %f\n", epsilon_);
}

template <int Dim> void MPBaseUtil<Dim>::setMaxNum(int num) {
  max_num_ = num;
  if (planner_verbose_)
    printf("[MPBaseUtil] set max num: %d\n", max_num_);
}

template <int Dim> void MPBaseUtil<Dim>::setDt(decimal_t dt) {
  ENV_->set_dt(dt);
  if (planner_verbose_)
...
template <int Dim> void MPBaseUtil<Dim>::setAlpha(int alpha) {
  ENV_->set_alpha(alpha);
  if (planner_verbose_)
    printf("[MPBaseUtil] set alpha: %d\n", alpha);
}

template <int Dim> void MPBaseUtil<Dim>::setU(const vec_Vecf<Dim> &U) {
  ENV_->set_U(U);
}

template <int Dim> void MPBaseUtil<Dim>::setVmax(decimal_t v_max) {
  ENV_->set_v_max(v_max);
  if (planner_verbose_)
    printf("[MPBaseUtil] set v_max: %f\n", v_max);
}

template <int Dim> void MPBaseUtil<Dim>::setAmax(decimal_t a_max) {
  ENV_->set_a_max(a_max);
  if (planner_verbose_)
    printf("[MPBaseUtil] set a_max: %f\n", a_max);
}

template <int Dim> void MPBaseUtil<Dim>::setJmax(decimal_t j_max) {
  ENV_->set_j_max(j_max);
  if (planner_verbose_)
    printf("[MPBaseUtil] set j_max: %f\n", j_max);
}

template <int Dim> void MPBaseUtil<Dim>::setUmax(decimal_t u_max) {
  ENV_->set_u_max(u_max);
  if (planner_verbose_)
    printf("[MPBaseUtil] set u_max: %f\n", u_max);
}

template <int Dim> void MPBaseUtil<Dim>::setTmax(decimal_t t) {
  // ENV_->set_t_max(t);
  max_t_ = t;
  if (planner_verbose_)
...
template <int Dim> vec_E<Waypoint<Dim>> MPBaseUtil<Dim>::getWs() const {
  return ws_;
}

template <int Dim> Trajectory<Dim> MPBaseUtil<Dim>::getTraj() const {
  return traj_;
}

template <int Dim> vec_Vecf<Dim> MPBaseUtil<Dim>::getOpenSet() const {
  vec_Vecf<Dim> ps;
...
template <int Dim> vec_Vecf<Dim> MPBaseUtil<Dim>::getCloseSet() const {
  vec_Vecf<Dim> ps;
  for (const auto &it : ss_ptr_->hm_) {
...
template <int Dim> vec_Vecf<Dim> MPBaseUtil<Dim>::getNullSet() const {
  vec_Vecf<Dim> ps;
  for (const auto &it : ss_ptr_->hm_) {
...
template <int Dim> vec_Vecf<Dim> MPBaseUtil<Dim>::getExpandedNodes() const {
  return ENV_->expanded_nodes_;
}

template <int Dim> int MPBaseUtil<Dim>::getExpandedNum() const {
  return ss_ptr_->expand_iteration_;
}

template <int Dim> void MPBaseUtil<Dim>::getSubStateSpace(int time_step) {
  ss_ptr_->getSubStateSpace(time_step);
}

template <int Dim> void MPBaseUtil<Dim>::checkValidation() {
  ss_ptr_->checkValidation(ss_ptr_->hm_);
}

template <int Dim>
bool MPBaseUtil<Dim>::plan(const Waypoint<Dim> &start,
