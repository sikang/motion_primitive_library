...
template <int Dim>
PolyGradientDescent<Dim>::PolyGradientDescent(
    unsigned int smooth_derivative_order, unsigned int minimize_derivative,
    bool debug)
...
bool PolyGradientDescent<Dim>::gradient_descent(vec_E<Waypoint<Dim>> waypoints,
  ...
  for (const auto &it : waypoints) {
    if (it.use_pos && smooth_derivative_order >= 0) {
      ...
      if (id > 0 && id < num_waypoints - 1)
        permutation_table.push_back(
            std::make_pair(raw_cnt + N / 2, num_fixed_derivatives + free_cnt));
      raw_cnt++;
      free_cnt++;
    }
  ...
  for (int i = 0; i < n; i++) {
    double J = gradient_descent(num_fixed_derivatives, num_free_derivatives, L,
                                T, deltaT, V, R, D);
...
template <int Dim>
double PolyGradientDescent<Dim>::gradient_descent(
    int num_fixed_derivatives, int num_free_derivatives, const MatDf &L,
    const MatDf &T, const MatDf &deltaT, const MatDf &V, const MatDf &R,
    MatDNf<Dim> &D) {
  ...
  for (int dim = 0; dim < Dim; dim++) {
    double alpha = 1;
    MatDf gk = 2 * Df.col(dim).transpose() * Rfp +
               2 * Dp.col(dim).transpose() * Rpp + Jc_d.row(dim);
    ...
    Cs = get_Jc(T, deltaT, V, L * D, Lpp);
    MatDf yk = (2 * Df.col(dim).transpose() * Rfp +
                2 * xk_new.transpose() * Rpp + Cs.second.row(dim) -
                gk).transpose();
    if (sk.norm() != 0) {
      B_[dim] += yk * yk.transpose() / (yk.transpose() * sk)(0, 0) -
                 B_[dim] * sk * sk.transpose() * B_[dim] /
                     (sk.transpose() * B_[dim] * sk)(0, 0);
    }
  }
...
template <int Dim>
double PolyGradientDescent<Dim>::get_c(const Vecf<Dim> &pt,
                                       const Vecf<Dim> &ref_pt) {
...
template <int Dim>
Vecf<Dim>
PolyGradientDescent<Dim>::get_c_derivative(const Vecf<Dim> &pt,
                                           const vec_Vecf<Dim> &ref_pts) {
...
template <int Dim> VecDf PolyGradientDescent<Dim>::getT(int N, double t) {
  VecDf vec(N);
  for (int i = 0; i < N; i++)
    vec(i) = power(t, i);
  return vec;
...
template <int Dim>
std::pair<MatDf, MatDf>
PolyGradientDescent<Dim>::get_Jc(const MatDf &T, const VecDf &deltaT,
                                 const MatDf &V, const MatDf &P,
                                 const MatDf &Lpp) {
  // f
  MatDf f = T * P;
  // v
  ...
  MatDf Jc_d = wc_ * deltaC.transpose() * T * Lpp + C.transpose() * T * V * Lpp;
