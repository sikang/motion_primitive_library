...
LambdaSeg::LambdaSeg(const VirtualPoint &v1, const VirtualPoint &v2) {
  Mat4f A;
  A << v1.t * v1.t * v1.t, v1.t * v1.t, v1.t, 1, 3 * v1.t * v1.t, 2 * v1.t, 1,
      0, v2.t * v2.t * v2.t, v2.t * v2.t, v2.t, 1, 3 * v2.t * v2.t, 2 * v2.t, 1,
      0;
  Vec4f b;
...
decimal_t LambdaSeg::getT(decimal_t t) const {
  return a(0) / 4 * t * t * t * t + a(1) / 3 * t * t * t + a(2) / 2 * t * t +
         a(3) * t;
}
...
bool Lambda::exist() const { return !segs.empty(); }
...
template <int Dim> decimal_t Trajectory<Dim>::getTotalTime() const {
  return total_t_;
}
...
template <int Dim> bool Trajectory<Dim>::scale(decimal_t ri, decimal_t rf) {
  std::vector<VirtualPoint> vs;
  VirtualPoint vi, vf;
  vi.p = 1.0 / ri;
  vi.v = 0;
  ...
  for (int id = 0; id < (int)segs.size(); id++) {
    if (tau >= taus[id] && tau <= taus[id + 1]) {
      ...
      for (int j = 0; j < Dim; j++) {
        ...
        p.pos(j) = d(0);
        p.vel(j) = d(1) / lambda;
        p.acc(j) = d(2) / lambda / lambda -
                   d(1) * lambda_dot / lambda / lambda / lambda;
        p.jrk(j) = d(3) / lambda / lambda -
                   3 / power(lambda, 3) * d(2) * d(2) * lambda_dot +
                   3 / power(lambda, 4) * d(1) * lambda_dot *
                       lambda_dot; // assume lambda_ddot = 0
      }
      return true;
    }
  }
...
template <int Dim> vec_E<Waypoint<Dim>> Trajectory<Dim>::sample(int N) const {
...
template <int Dim> Lambda Trajectory<Dim>::lambda() const { return lambda_; }

template <int Dim> decimal_t Trajectory<Dim>::J(int i) const {
  decimal_t J = 0;
  for (const auto &seg : segs)
    J += seg.J(i);
  return J;
...
template <int Dim> std::vector<decimal_t> Trajectory<Dim>::getSegsT() const {
