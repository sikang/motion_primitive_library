...
FunnelUtil::FunnelUtil(decimal_t kp, decimal_t kv, decimal_t v)
    : kp_(kp), kv_(kv), v_(v) {}
...
void FunnelUtil::set_region(const Vec3f &ori, const Vec3f &dim) {
  ...
  Vs.push_back(
      Face(ori + dim - Vec3f(0, dim(1) / 2, dim(2) / 2), Vec3f::UnitX()));
  Vs.push_back(
      Face(ori + dim - Vec3f(dim(0) / 2, 0, dim(2) / 2), Vec3f::UnitY()));
  Vs.push_back(
      Face(ori + dim - Vec3f(dim(0) / 2, dim(1) / 2, 0), Vec3f::UnitZ()));
...
bool FunnelUtil::isFree(const Primitive3D &pr, const Vec3f &x10, Vec3f &x1f) {
  if (!Vs_.empty()) {
  ...
  // decimal_t max_v = std::max(std::max(pr.max_vel(0), pr.max_vel(1)),
  // pr.max_vel(2));
